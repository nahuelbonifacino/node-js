...
app.put("/people/:index", (req, res) => {
   ...
   */
  const userId = parseInt(req.params.index);
  const nuevosDatos = req.body
  const numUserId = parseInt(userId)
  const recurso = people[numUserId]

  if (!(numUserId >= 0 && numUserId < people.length)) {
    return res.status(404).json({ message: "Usuario no encontrado" });
  }else{
    recurso.first_name = nuevosDatos.first_name;
    recurso.last_name = nuevosDatos.last_name
    recurso.email = nuevosDatos.email;
...
app.delete("/people/:index", (req, res) => {
   ...
   */
    ...
    /* const usuarioAEleminar = people.indexOf(p => p.id == userId); */
    if (numUserId >= 0 && numUserId < people.length) {
      let person = people[numUserId];
      people.splice(person, 1);
      res.status(200).json({ message: "Usuario eliminado"});
    } else {  
      return res.status(404).json({ message: "Usuario no escontrado"}); 
}});
